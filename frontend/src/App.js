...
function App() {
  ...
  const [message, setMessage] = useState(''); // Simple message state instead of toast
  ...
  // Simple message function instead of toast
  ...
  const loadRecipients = async (campaignId) => {
    try {
      ...
      showMessage("Failed to load recipients");
    }
  };
  ...
  const createCampaign = async () => {
    ...
    try {
      ...
      showMessage("Failed to create campaign");
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateAIContent = async () => {
    if (!aiForm.prompt) {
      showMessage("Please provide a prompt for AI content generation");
      return;
    }
    ...
    try {
      ...
      showMessage("Failed to generate AI content");
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const addRecipient = async () => {
    if (!currentCampaign || !recipientForm.email) {
      showMessage("Please select a campaign and provide an email");
      return;
    }
    ...
    try {
      ...
      showMessage("Failed to add recipient");
    } finally {
      setLoading(false);
    }
  };
  ...
  const uploadCSV = async () => {
    if (!currentCampaign || !csvFile) {
      showMessage("Please select a campaign and CSV file");
      return;
    }
    ...
    try {
      ...
      showMessage(`Added ${response.data.recipients_added} recipients. ${response.data.total_errors} errors.`);
      
      loadRecipients(currentCampaign.id);
      loadAnalytics(currentCampaign.id);
      setCsvFile(null);
    } catch (error) {
      showMessage("Failed to upload CSV");
    } finally {
      setLoading(false);
    }
  };
  ...
  const sendBulkEmail = async (testMode = false) => {
    if (!currentCampaign) {
      showMessage("Please select a campaign");
      return;
    }

    if (recipients.length === 0) {
      showMessage("Campaign has no recipients");
      return;
    }
    ...
    try {
      ...
      showMessage(`Email campaign started! ${testMode ? '(Test mode - first 5 recipients)' : ''}`);
      ...
      showMessage("Failed to send emails");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <div className="container mx-auto p-6">
        ...
        {/* Simple message display instead of toast */}
        {message && (
